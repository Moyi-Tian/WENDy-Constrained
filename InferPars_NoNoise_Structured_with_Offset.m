...
if save_results == 1
    ...
    fname_results = sprintf('WENDy with Structure Matrix and Offset Fully-Mixed Dynamics subsamp=%u, beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, t_grid_num=%u, E0=%.3g NO NOISE.mat',subsamp,pars(1),pars(2),pars(3),pars(4),pars(5),t_max,t_num,E0);
...
if write_to_txt == 1
    % Write output to txt
    logFile = 'results_fully_mixed_structured_with_offset_NoNoise.txt';
    fid = fopen(logFile,'a');
    if fid < 0
...
if save_fig == 1
    ...
    fig_name = sprintf('Inference Results with Structure Matrix and Offset for Fully-Mixed Model beta=%.3g, theta=%.3g, eta=%.3g, gamma_i=%.3g, gamma_p=%.3g, tmax=%u, t_grid_num=%u, E0=%.3g NO NOISE.pdf',pars(1),pars(2),pars(3),pars(4),pars(5),t_max,t_num,E0);
