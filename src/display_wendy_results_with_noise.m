...
% Starred quantities must be built from the CLEAN library Theta_cell_true
% (evaluated on xsub above), NOT the observed library Theta_cell. Using
% Theta_cell here makes G_0_true identical to G_0, which silently collapses
% F_obs_error to zero and removes the observation-error channel entirely.
...
% Phi * grad(F) * eps, i.e. the library Jacobian CONTRACTED WITH THE NOISE.
% Jac_mat(j,l,m) = dTheta_j/du_l at time m. For equation i the coefficient of
% feature j is w_full(j,i), with w_full the affine map S*theta + C. So
%
%     g_i(m) = sum_l [ sum_j w_full(j,i) * dTheta_j/du_l(m) ] * eps_l(m)
%
...
g = zeros(numeq, M);
for i = 1:numeq
    Ji     = squeeze(sum(Jac_mat .* w_full(:,i), 1));   % nstates x M
    g(i,:) = sum(Ji .* noise', 1);                      % contract with the noise
end
Jac_F_obs_error = blkdiag(V_cell{:}) * reshape(g', [], 1);
...
% CovW comes from wendy_fcn unchanged (upstream form). Note that it is the
% OLS-type covariance G_0^+ C G_0^+', not the GLS form (G_0' C^{-1} G_0)^{-1},
% even though w_hat is the GLS estimate; the two coincide only for square G_0.
% Substituting the GLS form is tempting but makes the bounds too NARROW: both
% expressions linearise the noise propagation and drop the O(eps^2) term, which
% is not negligible for coefficients multiplying bilinear features such as U*E
% and U*P. Against a Monte Carlo at sigma_NR = 0.05 the GLS form came out at
% 0.71 and 0.83 of the empirical spread for those two coefficients, while the
% upstream form was conservative (1.21, 1.20); both agreed within 10% for the
% coefficients on the linear features. Conservative bounds are the safer error,
% so the upstream form is kept.
