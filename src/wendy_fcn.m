...
%%%%%%%%%%%%   coefficient vector replaced by the affine map w = S*theta + C (both
%%%%%%%%%%%%   optional; see the help text below); whitened solve simplified; the
%%%%%%%%%%%%   resolved S and C returned as outputs.
%%%%%%%%%%%%
%%%%%%%%%%%%   NOTE ON THE FILENAME: this file derives from upstream wendy_fcn_0.m,
%%%%%%%%%%%%   not from upstream wendy_fcn.m. It is renamed here because this
%%%%%%%%%%%%   repository keeps only one solver, so the _0 suffix distinguishes
%%%%%%%%%%%%   nothing. Upstream's own wendy_fcn.m is the unstructured solver and is
%%%%%%%%%%%%   not included here, so do not diff this file against it.
...
% The full coefficient vector is the affine map
%
%       w = S*theta + C
%
% where theta is what gets estimated, S encodes dependencies between
% coefficients (parameter tying / sign flips / structural zeros) and C holds
% coefficients that are known a priori and are not estimated.
%
...
% downstream display scripts never have to re-derive them.
    ...
    %%% get dimensions
    featuresPerEqn  = repmat({ features }, numeq, 1);
    param_length_vec = cellfun(@numel, featuresPerEqn);

    %%% resolve the affine map, filling in defaults
    nfull = sum(param_length_vec);   % length of the full coefficient vector
    if isempty(S)
        S = eye(nfull);              % no structure: every coefficient is free
    end
    if isempty(C)
        C = zeros(nfull,1);          % no known offsets
    end
    S = full(S);
    C = full(C(:));
    if size(S,1) ~= nfull
        error('wendy_fcn:badS', ...
            'S has %d rows but the full coefficient vector has length %d (numeq*numel(features)).', ...
            size(S,1), nfull);
    end
    if numel(C) ~= nfull
        error('wendy_fcn:badC', ...
    ...
    Theta_mat = [ Theta_cell{:} ];
    Theta_mat_pre = repmat({Theta_mat},1,numeq);
    Theta_big = blkdiag(Theta_mat_pre{:});
    G_0_pre = V_big * Theta_big;

    %%% apply the affine map: G_0 acts on theta, and the known-offset
    %%% contribution C_0 moves to the right-hand side
    G_0 = blkdiag(G_0_pre*S);
    C_0 = blkdiag(G_0_pre*C);
    ...
    if max_iter>1
        [L0,L1] = get_Lfac(Jac_mat,param_length_vec,V_cell,Vp_cell,numeq);
        L0=L0*RT_0;
    ...
    while all([check>iter_diff_tol,iter<max_iter,pval>pvalmin])
        ...
        %%% update covariance
        [RT,~,~,~] = get_RT(L0,L1,S*w_hat+C,diag_reg);

        %%% whiten the system. b_0 already carries the known-offset
        %%% correction (b_0 = -Vp*x - C_0, set above), so no further
        %%% offset bookkeeping is needed here: RT\(b_0+C_0) - RT\C_0
        %%% is just RT\b_0 by linearity of the solve.
        G = RT \ G_0;
        b = RT \ b_0;
    ...
    if pval<pvalmin
        ...
        elseif ind<size(w_hat_its,2)
            % Covariance is built from the PREVIOUS iterate (ind-1), matching
            % the unconstrained WENDy reference. Full coefficient vector is
            % the affine map S*theta + C.
            [RT,~,~,~] = get_RT(L0,L1,S*w_hat_its(:,ind-1)+C,diag_reg);
        end
        res = [res res(:,ind)];
