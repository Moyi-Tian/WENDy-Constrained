...
function [V,Vp] = VVp_svd(V,K_min,t,toggle_VVp_svd)
    ...
    if toggle_VVp_svd>0
        ...
        if isempty(s)
            % was: s = min(K,size(V,1)); K is not defined in this scope, so this
            % branch errored. If the threshold is never reached, no truncation is
            % warranted: keep every available singular vector.
            s = size(U,2);
        end
    else
